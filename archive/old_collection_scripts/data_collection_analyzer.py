...
class ParameterOptimizer:
    ...
    def optimize_buffer_parameters(self):
        """优化buffer参数"""
        if self.df.empty or 'near_lower' not in self.df.columns:
            print("⚠️ 无法优化buffer参数，缺少必要数据")
            return {}
        
        # 分析不同buffer参数对near_lower的影响
        print("🔍 分析不同buffer参数的影响...")
        
        # 模拟不同参数组合
        atr_multipliers = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
        min_buffers = [0.001, 0.005, 0.01, 0.015, 0.02]
        ...
        for mult in atr_multipliers:
            for min_buf in min_buffers:
                ...
                # 评估效果：希望near_lower为True时，实际价格确实接近下轨
    ...
    def visualize_data(self):
        ...
        if 'rsi_1m' in self.df.columns and 'rsi_5m' in self.df.columns:
            axes[1, 0].hist(self.df['rsi_1m'].dropna(), bins=30, alpha=0.5, label='RSI 1m', density=True)
            axes[1, 0].hist(self.df['rsi_5m'].dropna(), bins=30, alpha=0.5, label='RSI 5m', density=True)
            axes[1, 0].axvline(x=30, color='red', linestyle='--', label='RSI 30 (超卖)')
            axes[1, 0].axvline(x=70, color='green', linestyle='--', label='RSI 70 (超买)')
            axes[1, 0].set_title('RSI分布')
            axes[1, 0].set_xlabel('RSI值')
            axes[1, 0].legend()
        
        # 4. near_lower决策分布
        if 'near_lower' in self.df.columns:
            near_lower_counts = self.df['near_lower'].value_counts()
            axes[1, 1].bar(['False', 'True'], [near_lower_counts.get(False, 0), near_lower_counts.get(True, 0)])
            axes[1, 1].set_title('near_lower决策分布')
...
def enhance_strategy_with_logging():
    ...
    def enhanced_grid_trading_strategy_pro1():
        ...
        buffer = max(0.1 * (atr if atr else 0), 0.005)  # 使用优化后的参数
...
def run_optimization_process():
    ...
    # 4. 加载现有数据并分析
    print("📈 加载现有数据并进行分析...")
    analysis_results = optimizer.analyze_performance()
    
    # 5. 优化参数
    print("🔍 开始参数优化...")
    best_params = optimizer.optimize_buffer_parameters()

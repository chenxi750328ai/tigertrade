...
**每日目标（实盘）**：**胜率 > 60%** 且 **实盘收益率达标**（今日有成交且当日收益 ≥ 0，月盈利率 20%）。每天都要有目标，**不达到目标不停息**；任一项未达标则继续优化/执行直至达标或当日不可抗力（休市等）并记录。详见 [盈利目标和风险控制](../../docs/盈利目标和风险控制.md)「实盘每日目标」。

**按角色**：设计 / 开发 / 测试 / Leader 的**每日例行工作**与职责定义见 [项目角色与每日例行工作](项目角色与每日例行工作.md)。**Leader（陈正霞）** 除本人例行外，须**监控项目计划执行、各成员进展与问题**；每日/收工须对照 [项目计划_月度周计划](../../docs/项目计划_月度周计划.md) 填写 [例行汇报_日周月对齐_陈正霞](../../docs/reports/例行汇报_日周月对齐_陈正霞.md)（日目标 ↔ 周/月一致），并刷新 [routine_pulse_latest](../../docs/reports/routine_pulse_latest.md)。
...
6. **收益率分析和算法优化**（两项都做，不得只做其一）
   ...
   - **报告自检以网页为准**：报告自检是否通过须到**部署后的网页**查看（如 GitHub Pages 的 status、策略报告页），**本地生成的 docs/reports、docs/status.html 不算**；push 后等待 Pages 部署完成，再打开网页核对内容是否最新。详见 [每日例行_效果数据说明](../../docs/每日例行_效果数据说明.md)。
   - **报告空项必须写明根因**：实际收益率（老虎核对）、推算收益率等为空时，**必须在报告内写明原因**（如算法报告中的「本报告空项根因说明」），不得只写「—」；后台数据拉取失败要报出具体原因（如 api_manager 未初始化、openapicfg_dem 不可用）。自检时须检查空项根因是否写清；所有问题追根问底、扎到根因，不忽悠。
...
7. **状态页刷新**
   ...
   - **报告/状态自检**：自检须在**网页**上完成（打开部署后的 Pages URL，如 `https://<owner>.github.io/tigertrade/status.html`、`.../reports/strategy_reports_index.html`），本地打开 docs 不算；若网页仍为旧内容，需排查部署或缓存。
...
- **收尾**：未完成项需注明原因与下次计划。
- **推代码前**：必须本地跑与 CI 同一套测试命令（见上表「测试」），通过后再推送；**并**执行 `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python scripts/verify_design_completeness_and_dfx.py`（或与 CI「Design completeness + DFX gate」等价），避免「本地过 CI 挂」或设计文档/DFX 回归被删。根因与改进见 `insights/CI为何通过与防止再犯_20260202.md`。
- **设计完备性（DFX，必须可执行验收）**  
  - **依据**：[设计完备性_业界方案与DFX](../../docs/设计完备性_业界方案与DFX.md)（尤其 §3.4 可服务性「问题可定位、可追溯」、§3.7 可测试性）；[需求分析和Feature测试设计](../../docs/需求分析和Feature测试设计.md)。  
  - **硬门禁（落地）**：推前或合并前在仓库根 `tigertrade/` 执行：  
    `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python scripts/verify_design_completeness_and_dfx.py`  
    该脚本会：**(1)** 校验下列文档文件存在（缺失则 exit 2）：`docs/设计完备性_业界方案与DFX.md`、`docs/需求分析和Feature测试设计.md`、`docs/reports/日志文件在哪儿.md`、`docs/reports/止损未触发_取证与归因.md`；**(2)** 跑通 DFX/组合单回归用例（`tests/test_design_completeness_dfx_gate.py`、`tests/test_executor_dfx_emission.py`、`tests/test_order_log_dfx.py`、`tests/test_api_adapter_bracket_and_fill.py`）。  
  - **CI**：主工作流已含同名步骤「Design completeness + DFX gate」，与上脚本一致；**不得**仅以「文档在仓库里」代替脚本与用例通过。  
  - **说明**：不在状态页单独占一行；但本项与第 1 项 CI 测试同为发布前必过项。
...
**请勿遗忘：例行工作 = CI/CD 测试 + 覆盖率统计 + 问题解决 + 20 小时稳定性测试（含 20 小时 DEMO 运行）+ 数据刷新和模型训练（含多模型对比训练）+ 收益率分析和算法优化（收益率分析 + 算法优化，两项都做）+ 状态页刷新。每日目标 = 实盘胜率 > 60% + 实盘今日有成交且收益≥0（月盈利率 20%），不达标不停息。**

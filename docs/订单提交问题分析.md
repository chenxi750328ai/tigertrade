...
```
code=1010 msg=biz param error(account '21415812702670778' is not authorized to the api user)
```

### 1.2 实际情况

- ✅ API调用成功（不是网络问题）
- ✅ account参数传递正确（account=21415812702670778）
- ✅ Order对象创建成功
- ❌ Tiger API返回：account没有授权给API用户
...
2. **授权问题**：
   - Tiger API要求：account必须授权给API用户（tiger_id）
   - 当前状态：account `21415812702670778` 没有授权给API用户 `20157140`
   - 需要在Tiger后台配置授权
...
RealTradeApiAdapter.place_order()
  ↓
创建Order对象（account=21415812702670778）
  ↓
...
1. 登录Tiger后台
2. 找到API管理/账户授权
3. 将account `21415812702670778` 授权给API用户 `20157140`
4. 保存配置

...
**必守**：**DEMO 实盘收益率 = 仅以老虎后台可核对的数据为准**。完成的订单才计入收益率；未在老虎后台成交的（模拟单、下单失败、仅日志统计）不得作为实盘收益率展示。详见 [DEMO实盘收益率_定义与数据来源](DEMO实盘收益率_定义与数据来源.md)。**每日期望**：当日 DEMO 能产生至少一笔可在老虎后台核对的订单，使报告中出现实际收益率/胜率（而非全为 —）。
...
| **5. 为何报告里后台核对数据都是空？** | 见 [为何报告里后台核对数据为空_说明](为何报告里后台核对数据为空_说明.md)：主要是生成报告时**没有从老虎 API 拉订单**（或未解析）；后台订单来源与「没有核对 vs 没有符合」说明在该文档。 |
...
3. **报告**
   ...
   - **报告自检**：生成后会自检是否合理——算法报告检查「回测仅 1 笔时是否含胜率说明」「无 API 时是否标明数据来源」；策略对比报告检查「实盘/DEMO 胜率列是否仅来自 API（无 API 时不为 100%）」。通过或警告会打在运行日志中。
   - **报告自检须到网页上查看，本地不算**：自检是否通过以**部署后的网页**为准（如 GitHub Pages 部署完成后的状态页、策略报告页）。本地生成的 `docs/reports/`、`docs/status.html` 仅表示本次生成结果，**须在 push 后等待 Pages 部署完成，再打开网页核对**；若网页内容仍为旧，说明部署未生效或缓存未更新，需排查部署与缓存。网页地址示例：`https://<owner>.github.io/tigertrade/status.html`、`https://<owner>.github.io/tigertrade/reports/strategy_reports_index.html`（以仓库实际 Pages 设置为准）。
   - **所有空项必须写明根因，不得留白**：报告里若实际收益率（老虎核对）、推算收益率等为空，**必须在报告内写明原因**（如「本报告空项根因说明」），不能只写「—」让用户猜。后台数据拉取失败也要报出原因（如 api_manager 未初始化、openapicfg_dem 不可用等）。所有问题追根问底、扎到根因，不忽悠。

...
**金融系统要求高可靠性**；**20 小时稳定运行不出错**是项目目标之一，用于验证系统具备长时间无故障运行能力，**不是跑一次即可**。例行中须确保 DEMO 启动、运行满 20h（或约定时长），并检查运行结果（无崩溃、错误率等达标）。

项目已建立完整的CI/CD和稳定性测试流程，包括：
1. **每日CI/CD测试** - 自动运行测试套件和覆盖率检查
2. **20小时稳定性测试** - 长时间运行验证系统稳定性（**可靠性目标**，须运行满 20h 并产出结果）
3. **问题检测和修复** - 自动分析问题并生成修复建议
4. **数据分析和优化** - 基于测试数据优化算法

### 20h 与主 CI 的关系（QA-D3/T1）

- 主 CI（push/每日）不跑满 20h，仅短时冒烟，不阻塞合并。
- 20h 由独立 workflow 或定时任务触发（如 `scripts/stability_test_20h.py`），产出 stability_report 等。
- 未跑满 20h 时主 CI 不 block；TC-F6-001 由稳定性流水线执行并检查结果，不在主 pytest 内跑 20h。
...
### 稳定性目标（可靠性目标，与需求一致）
- **20 小时无崩溃**：系统持续运行满 20 小时不因未处理异常退出，**是项目可靠性目标**，例行须验证。
- **错误率**: < 1%
- **内存使用**: < 1GB
- **CPU使用**: < 80%
- 运行须有**可用输出**（日志、订单/统计），供收益与稳定性分析；无输出或中途崩溃视为异常。

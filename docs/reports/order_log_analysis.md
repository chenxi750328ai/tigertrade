...
- mode=**real**, status=**fail**: 907 条
- mode=**real**, status=**success**: 642 条
...
| 可能为老虎真实单（order_id 为纯数字） | 578 | **未与老虎核对**，可能含测试污染；真实成功数仅以老虎后台为准。 |
...
- **本地风控拒绝**（未发 API）：846 条（如 ALLOW_REAL_TRADING、持仓硬顶）
- **真实 API 被拒**（老虎拒绝）：61 条（如 1010/1200/account 为空）

### mode=real 且 status=fail 的典型错误（前 15 条）

- `ALLOW_REAL_TRADING!=1`: 808 次
- `order_id无效(来自mock/测试)，未记入实盘成功`: 21 次
- `API Error 1010: biz param error(field 'account' cannot be empty)`: 21 次
- `持仓硬顶 pos=999>=3`: 17 次
...
- 2026-02-24T16:59:22.585401
- 2026-02-25T17:21:12.468787
- 2026-02-25T17:31:43.729957
- 2026-02-25T17:33:30.667944

...
生成时间: 2026-02-27T15:36:21.948986
...
- **实际收益率（老虎后台核对）为空** → 原因：无（本次已从老虎拉取到订单并解析）
...
从后台拉取订单（及可选持仓）后的分析结果，用于发现**有持仓但止损/止盈单已撤**（可能保证金不足或强平）等情况。详见 [后台订单与持仓分析_有仓无止损止盈_风控改进](../后台订单与持仓分析_有仓无止损止盈_风控改进.md)。

- 持仓数量（来源）: 2 手（get_positions）
- 未撤止损/止盈单: 0 笔；已撤/拒/过期: 0 笔

**⚠️ 风控建议**: 检测到后台持仓 2 手，未发现有效止损/止盈单（可能被系统撤单或未提交）。建议检查风控与补单逻辑。

## 实盘收益率：实际（老虎核对）与推算（未核对）

| 项目 | 值 | 说明 |
| --- | --- | --- |
| **实际收益率（老虎后台核对）** | +7945.90 USD | 仅老虎后台订单/成交计算；未拉取或未核对时见上方空项根因。 |
...
| --- | --- | --- | --- | --- | --- |
| moe_transformer | 100.0 | 7.01 | 0.0 | 0.03 | 98.0 |
| lstm | 100.0 | 7.01 | 0.0 | 0.03 | 98.0 |
| grid | 7.0 | 27.92 | 3.99 | 7.21 | 100.0 |
| boll | 2.0 | 7.52 | 3.76 | 8.38 | 50.0 |

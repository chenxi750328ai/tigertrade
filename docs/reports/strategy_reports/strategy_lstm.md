...
*报告生成时间：2026-02-24T16:11:04.344542*
...
| return_pct | 0.99 | (期末资金 − 10万) / 10万 × 100（%）。来自 data/processed/test.csv 历史 K 线回测。 |
| avg_per_trade_pct | 0.0 | 总收益/笔数，每笔占初始资金%。 |
| top_per_trade_pct | 0.01 | 单笔最大收益占初始资金%。 |
...
| 指标 | 值 | 说明 |
| --- | --- | --- |
| win_rate | 28.6 | 盈利笔数 / 完成笔数 × 100（%）。回测表为回测结果；实盘表为实盘胜率，仅来自 API 历史订单解析。 |
| yield_verified | +5876.90 USD | 用老虎后台订单/成交数据计算出的收益率；未拉取或未核对时为 —。 |
| yield_estimated | — | 未与老虎核对时的推算值（如 API 报告解析）；无推算时为 —。 |
| today_yield_pct | +5876.90 USD | 本日在状态/报告中展示的收益率，来自 today_yield.json；须以实际（老虎核对）为准。 |
| profitability | 0 | API 历史订单解析得到的总交易数、总盈亏等；无 API 时为 0 或 —。 |
| demo_order_success | 10029 | DEMO 日志中「订单提交成功」等匹配次数（多日志汇总），非老虎后台笔数。 |
| demo_sl_tp_log | 91990 | 日志全文匹配「止损|止盈|已提交止损|已提交止盈」等的出现次数。 |
| demo_execute_buy_calls | 34938 | 日志匹配「execute_buy|动作: 买入」的次数。 |
| demo_success_orders_sum | 336533 | 日志内统计的成功订单数汇总，非老虎后台。 |
| demo_fail_orders_sum | 4582079 | 日志内统计的失败订单数汇总。 |
| demo_logs_scanned | 27 | 参与汇总的 demo_*.log、demo_run_20h_*.log 文件个数。 |

...
class HugeTransformer(nn.Module):
    """超大型Transformer模型 - 使用更多参数和更深的网络结构"""
    def __init__(self, input_size=12, d_model=512, nhead=8, num_layers=8, output_size=3, dropout_rate=0.1):
        super(HugeTransformer, self).__init__()
        ...
        # 位置编码（可学习）
        self.pos_encoding = nn.Parameter(torch.zeros(1, 100, d_model))  # 最大序列长度100
        
        # 多层Transformer编码器
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=d_model,
            nhead=nhead,
            dim_feedforward=d_model * 4,  # FFN中间层是d_model的4倍
    ...
    def forward(self, x):
        ...
        x = x + pos_enc  # (batch_size, seq_len, d_model)
...
class HugeTransformerStrategy:
    ...
    def __init__(self, data_dir='/home/cx/trading_data', model_path=None):
        ...
        print(f"Huge Transformer参数数量: {total_params:,}")
        print(f"可训练参数数量: {trainable_params:,}")
        
        # 优化器和损失函数
        self.optimizer = torch.optim.AdamW(
            self.model.parameters(), 
            lr=0.0005,  # 稍微降低学习率以适应大模型
            weight_decay=1e-5,
            betas=(0.9, 0.999)
        )
    ...
    def train_model(self, df):
        ...
        try:
            ...
            for epoch in range(20):  # 增加到20轮训练
                ...
                print(f"超大Transformer训练轮次 {epoch+1}/20")
                ...
                if val_avg_loss < best_val_loss:
                    ...
                    print(f"  🏆 超大Transformer新最佳验证损失: {best_val_loss:.4f}")
                
                self.model.train()  # 重新设置为训练模式以进行下一轮训练
        
        except Exception as e:
            print(f"超大Transformer训练过程错误: {e}")
            import traceback
            traceback.print_exc()
        ...
        while self.should_train:
            try:
                ...
                if df is not None and len(df) > 0:
                    print(f"开始训练超大Transformer模型，数据量: {len(df)}")
                    with self.model_lock:
                        self.train_model(df)
                    print("超大Transformer模型训练完成")
                
                # 训练较慢，每30分钟训练一次
                time.sleep(1800)
                
            except Exception as e:
                print(f"超大Transformer训练线程错误: {e}")
                import traceback
                traceback.print_exc()
    ...
    def save_model(self, path):
        ...
        with self.model_lock:
            ...
            print(f"超大Transformer模型已保存到 {path}")


def main():
    parser = argparse.ArgumentParser(description='Huge Transformer Trading Strategy')
    ...
    strategy = HugeTransformerStrategy(model_path=args.model_path)
    
    if args.mode == 'train':
        print("仅运行超大Transformer模型训练模式...")
        # 训练模式，不退出
        try:
            while True:
                time.sleep(60)
        except KeyboardInterrupt:
            print("超大Transformer训练已停止")
    elif args.mode == 'predict':
        print("运行超大Transformer模型预测模式...")
        # 演示如何使用
        sample_data = {
            'price_current': 93.5,
            'grid_lower': 93.0,

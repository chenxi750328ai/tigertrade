...
def adjust_grid_interval(trend, indicators):
    ...
    if '5m' in indicators and 'boll_upper' in indicators['5m'] and 'boll_lower' in indicators['5m']:
        ...
        print(f"📈 网格参数已更新 - 上轨: {grid_upper:.3f}, 下轨: {grid_lower:.3f}, ATR: {atr_value:.3f}")
    else:
...
def place_tiger_order(side, quantity, price, stop_loss_price=None, take_profit_price=None, tech_params=None, reason=''):
    ...
    if api_manager.is_mock_mode:
        ...
        try:
            ...
            print(f"✅ [实盘单] 下单成功 | {side} {quantity}手 | 订单ID：{order_id}")
...
if __name__ == "__main__":
    ...
    # 解析命令行参数
    count_type = sys.argv[1] if len(sys.argv) > 1 and sys.argv[1] in ('d', 'c') else 'd'
    strategy_type = sys.argv[2] if len(sys.argv) > 2 and sys.argv[2] in ('backtest', 'llm', 'grid', 'boll', 'compare', 'large', 'huge', 'all') else 'all'
    ...
    if strategy_type == 'optimize':
        ...
        print("🚀 启动LLM交易策略...")
        # 初始化LLM交易策略
        llm_strat = llm_strategy.LLMTradingStrategy()
        ...
        while True:
            try:
                ...
                print(f"🧠 LLM模型预测: {action_map[action]}, 置信度: {confidence:.3f}")
                print(f"📊 比较 | 价={price_current:.3f}, ATR={atr:.3f}, 网格=[{grid_lower:.3f},{grid_upper:.3f}]")
                print(f"   条件详情: BUFFER={buffer:.3f}, 近轨={current_data['near_lower']}, RSI_OK={current_data['rsi_ok']}")
